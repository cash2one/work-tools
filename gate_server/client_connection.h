...
protected:
    ...
    int process_acc_login(const packet_info* pi) ;
    int process_player_login(const packet_info* pi) ;
    int process_create_player(const packet_info* pi) ;
    int process_heartbeat(const packet_info* pi) ;
	int process_find_player(const packet_info* pi);
	int process_random_name(const packet_info* pi);
	int process_init_connection(const packet_info* pi);
	int process_server_status(const packet_info* pi);
