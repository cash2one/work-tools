...
int day_roll_logger::init(const char* prefix,int log_level,int buf_size)
{
    ...
    if ( log_level < LOG_LEVEL_ERROR ) log_level = LOG_LEVEL_ERROR;
    if( log_level > LOG_LEVEL_DEBUG ) log_level = LOG_LEVEL_DEBUG ;
...
int day_roll_logger::write_format(int ll,const char* fmt,...)
{
    ...
    if ( m_fd < 0 || ll > m_level || ll < LOG_LEVEL_ERROR ) return -1 ;
