	...
	: file_name_(file_name)
{
	...
	file_name_ = file_name_ + ".dat";
	int status = mkdir(get_path().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
void namevalue_saver::copy_file(const string& source, const string& destination)
{
	...
	if (!stat(source.c_str(), &sourceStat)) {
	   int result = sendfile(output, input, &bytesCopied, sourceStat.st_size);
	}
